...
BYTE init_mbr(void) {
	...
	if (Buffer[510] == MASTER_BOOT_RECORD_ID1 && Buffer[511]
		...
		MBR_BS_Location = Buffer[454] + (Buffer[455] << 8)
				+ (Buffer[456] << 16) + (Buffer[457] << 24);
		MBR_Partition_Len = Buffer[458] + (Buffer[459] << 8) + (Buffer[460]
				<< 16) + (Buffer[461] << 24);
		return 0; //OK
	} else {
		return 1; //Error
...
BYTE init_bs(void) {
	...
	if (Buffer[510] == MASTER_BOOT_RECORD_ID1 && Buffer[511]
		...
		BPB_HiddSec = Buffer[28] + (Buffer[29] << 8) + (Buffer[30] << 16)
				+ (Buffer[31] << 24);
		BPB_TotSec32 = Buffer[32] + (Buffer[33] << 8) + (Buffer[34] << 16)
				+ (Buffer[35] << 24);
		...
		if (BPB_FATSz16 != 0) {
			...
			BS_VOLID_16 = Buffer[39] + (Buffer[40] << 8) + (Buffer[41] << 16)
					+ (Buffer[42] << 24);
...
UINT32 search_for_filetype(BYTE *extension, data_file *df, int sub_directory,
	...
	while ((Buffer[entry_num * 32] != 0x00)) {
		...
		if (((Buffer[entry_num * 32 + attribute_offset] & ATTR_LONG_NAME_MASK)
				== ATTR_LONG_NAME)
				&& (Buffer[entry_num * 32 + attribute_offset] != 0x08)
				&& (Buffer[entry_num * 32] != 0xE5)) //long filename
		{
			...
			while (longname_blocks > 0) {
				...
				longname[(longname_blocks - 1) * 13 + 1] = Buffer[entry_num
						* 32 + 3];
				longname[(longname_blocks - 1) * 13 + 2] = Buffer[entry_num
						* 32 + 5];
				longname[(longname_blocks - 1) * 13 + 3] = Buffer[entry_num
						* 32 + 7];
				longname[(longname_blocks - 1) * 13 + 4] = Buffer[entry_num
						* 32 + 9];
				longname[(longname_blocks - 1) * 13 + 5] = Buffer[entry_num
						* 32 + 14];
				longname[(longname_blocks - 1) * 13 + 6] = Buffer[entry_num
						* 32 + 16];
				longname[(longname_blocks - 1) * 13 + 7] = Buffer[entry_num
						* 32 + 18];
				longname[(longname_blocks - 1) * 13 + 8] = Buffer[entry_num
						* 32 + 20];
				longname[(longname_blocks - 1) * 13 + 9] = Buffer[entry_num
						* 32 + 22];
				longname[(longname_blocks - 1) * 13 + 10] = Buffer[entry_num
						* 32 + 24];
				longname[(longname_blocks - 1) * 13 + 11] = Buffer[entry_num
						* 32 + 28];
				longname[(longname_blocks - 1) * 13 + 12] = Buffer[entry_num
						* 32 + 30];
			...
			if (!strcmp(extension, fileext)) {
				if (file_count == file_number) {
					...
					df->Clus = (Buffer[entry_num * 32 + FstClusLo_offset])
							+ (Buffer[entry_num * 32 + FstClusLo_offset + 1]
									<< 8) + (Buffer[entry_num * 32
							+ FstClusHi_offset]) + (Buffer[entry_num * 32
							+ FstClusHi_offset + 1] << 8);
					df->FileSize = (Buffer[entry_num * 32 + FileSize_offset])
							| (Buffer[entry_num * 32 + FileSize_offset + 1]
									<< 8) | (Buffer[entry_num * 32
							+ FileSize_offset + 2] << 16) | (Buffer[entry_num
